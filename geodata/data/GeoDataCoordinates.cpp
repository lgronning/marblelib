...
#include "geodata/data/GeoDataCoordinates_p.h"
...
GeoDataCoordinates::GeoDataCoordinates(qreal _lon, qreal _lat, qreal _alt, GeoDataCoordinates::Unit unit)
  : d( new GeoDataCoordinatesPrivate( _lon, _lat, _alt, unit ) )
{
    d->ref.ref();
}
...
QString GeoDataCoordinates::toString( GeoDataCoordinates::Notation notation, int precision ) const
{
        ...
        if( notation == GeoDataCoordinates::UTM ){
            ...
            QString zoneString = ( zoneNumber > 0 ) ? QString::number(zoneNumber) : QLatin1String("");
            ...
            return QStringLiteral("%1%2 %3 m E, %4 m N").arg(zoneString).arg(bandString).arg(eastingString).arg(northingString);
        }
        else{
                                                    ...
                                                    char format )
{
    if( notation == GeoDataCoordinates::UTM ){
        ...
        // Handle lack of UTM zone number in the poles
        QString result = ( zoneNumber > 0 ) ? QString::number(zoneNumber) : QLatin1String("");
    ...
    if ( notation == DMS || notation == DM ) {
        ...
        if ( notation == DMS || precision < 3 ) {
            lonString += QStringLiteral(" %2\'").arg(lonMin, 2, 10, QChar('0') );
        }
        ...
        if ( notation == DMS ) {
            // Includes -1 case!
            if ( precision < 5 ) {
                lonString += QStringLiteral(" %3\"").arg(lonSec, 2, 'f', 0, QChar('0') );
                return lonString + weString;
            }

            lonString += QStringLiteral(" %L3\"").arg(lonSecF, precision - 1, 'f', precision - 4, QChar('0') );
        }
        else {
            lonString += QStringLiteral(" %L3'").arg(lonMinF, precision + 1, 'f', precision - 2, QChar('0') );
        }
    }
                                                    ...
                                                    char format )
{
    ...
    if ( notation == DMS || notation == DM || notation == Astro) {
        ...
        if ( notation == DMS || notation == Astro || precision < 3 ) {
            latString += QStringLiteral(" %2\'").arg(latMin, 2, 10, QChar('0') );
        }
        ...
        if ( notation == DMS || notation == Astro ) {
            // Includes -1 case!
            if ( precision < 5 ) {
                latString += QStringLiteral(" %3\"").arg(latSec, 2, 'f', 0, QChar('0') );
                return latString + nsString;
            }

            latString += QStringLiteral(" %L3\"").arg(latSecF, precision - 1, 'f', precision - 4, QChar('0') );
        }
        else {
            latString += QStringLiteral(" %L3'").arg(latMinF, precision + 1, 'f', precision - 2, QChar('0') );
        }
    }
...
Quaternion GeoDataCoordinates::quaternion() const
{
    return Quaternion::fromSpherical( d->m_lon, d->m_lat );
}

GeoDataCoordinates
GeoDataCoordinates::interpolate( const GeoDataCoordinates &target, double t_ ) const
{
...
bool GeoDataCoordinates::isPole(double lat, GeoDataCoordinates::Unit unit, Pole pole)
{
    qreal halfCircle;
    if ( unit == GeoDataCoordinates::Radian ) {
        halfCircle = M_PI;
    ...
    if ( fabs( (qreal) 2.0 * lat ) < halfCircle ) {
        return false;
    }
    else {
        if ( fabs( (qreal) 2.0 * lat ) == halfCircle ) {
            ...
            if ( pole == AnyPole ){
                ...
                if ( pole == NorthPole && 2.0 * lat == +halfCircle ) {
                    return true;
                }
                if ( pole == SouthPole && 2.0 * lat == -halfCircle ) {
                    return true;
                }
                return false;
            }
        ...
        // 
            ...
            // 90 deg S ... 90 deg N
            if ( fabs( (qreal) 2.0 * normalizeLat( lat, unit ) ) < halfCircle  ) {
                return false;
            }
            else {
                // Ok, we have hit a pole. Now let's check whether it's the one we've asked for:
