...
QDebug
operator<<(QDebug stream, const Interval &interval)
{
    return stream << "Interval(" << interval.start << "," << interval.end << ")";
}

// Compares two intervals according to their staring time.
// This is needed for sorting the intervals using library
// function SortHelper::sort(). See http://goo.gl/iGspV
...
void mergeIntervals(QList<Interval> &ranges)
{
    ...
    SortHelper::sort(ranges.begin(), ranges.end(), compareInterval);
...
GeoDataLatLonBox GeoDataLatLonBox::united( const GeoDataLatLonBox& other ) const
{
    ...
    QList<Interval> ranges;
    if(d->m_east < d->m_west)
    {
        ranges.append(Interval(-M_PI, d->m_east));
        ranges.append(Interval(d->m_west, M_PI));
    }
    else
    {
        ranges.append(Interval(d->m_west, d->m_east));
    }

    if(other.d->m_east < other.d->m_west)
    {
        ranges.append(Interval(-M_PI, other.d->m_east));
        ranges.append(Interval(other.d->m_west, M_PI));
    }
    else
    {
        ranges.append(Interval(other.d->m_west, other.d->m_east));
    }


    qDebug() << "before merged ranges" << ranges;

    mergeIntervals(ranges);

    qDebug() << "merged ranges" << ranges;

    if(ranges.size() == 1)
    {
        result.setWest(ranges.first().start);
        result.setEast(ranges.first().end);
    }
    else
    {
        ...
        for(int i = 0; i < ranges.size(); ++i)
        {
            ...
            if(i == 0)
            {
                distances.append(qAbs(ranges.last().end - range.start - (2*M_PI)));
            }
            else
            {
                distances.append(range.start- ranges[i-1].end);
            }
        }
        ...
        if(maxDistance == 0)
        {
            result.setWest(ranges.front().start);
            result.setEast(ranges.last().end);
        }
        else
        {
            result.setEast(ranges[maxDistance-1].end);
            result.setWest(ranges[maxDistance].start);
        }
    }
...
GeoDataLatLonBox GeoDataLatLonBox::fromLineString(  const GeoDataLineString& lineString  )
{
    ...
    qreal lon, lat;
    lineString.at(0).geoCoordinates( lon, lat );
    ...
    for(int i = 0; i < lineString.size(); ++i)
    {
        ...
        lineString.at(i).geoCoordinates( lon, lat );
        GeoDataCoordinates::normalizeLonLat( lon, lat );
        ...
        if( lineString.isClosed() && i == lineString.size()-1 )
        {
            i = 0;
