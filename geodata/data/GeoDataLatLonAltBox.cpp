...
GeoDataLatLonAltBox GeoDataLatLonAltBox::fromLineString(  const GeoDataLineString& lineString  )
{
    ...
    const qreal altitude = lineString.altitude(0);
    ...
    for ( ; it <= itEnd; ++it )
    {
        ...
        const qreal altitude = lineString.altitude(it);
...
QString GeoDataLatLonAltBox::toString( GeoDataCoordinates::Unit unit ) const
{
    ...
    case GeoDataCoordinates::Radian:
        return QStringLiteral( "North: %1; West: %2 MaxAlt: %3\n South: %4; East: %5 MinAlt: %6" )
	    .arg( north() ).arg( west() )
    ...
    return QStringLiteral( "GeoDataLatLonAltBox::text(): Error in unit: %1\n" )
	.arg( unit );
}
