    ...
    if ( texColorizer || m_radius != radius ) {
        ...
        for ( int tileY = minTileY; tileY <= maxTileY; ++tileY ) {
            for ( int tileX = minTileX; tileX <= maxTileX; ++tileX ) {
                ...
                const int deltaLevel = stackedId.tileLevel() - tile->id().tileLevel();
                ...
                const QImage part = toScale->copy( startX, startY, partWidth, partHeight ).scaled( toScale->size(), Qt::IgnoreAspectRatio, Qt::SmoothTransformation );
        ...
        for ( int tileY = minTileY; tileY <= maxTileY; ++tileY ) {
            for ( int tileX = minTileX; tileX <= maxTileX; ++tileX ) {
                ...
                const TileId cacheId = TileId( cacheHash, stackedId.tileLevel(), stackedId.x(), stackedId.y() );
...
void TileScalingTextureMapper::removePixmap( const TileId &tileId )
{
    ...
    for ( int i = 0; i < 4; ++i ) {
        const TileId id = TileId( i, stackedTileId.tileLevel(), stackedTileId.x(), stackedTileId.y() );
