...
#include "service/requester/RequestClient.h"
...
HttpDownloadManager::Private::Private(HttpDownloadManager *parent, StoragePolicy *policy )
    : m_downloadManager( parent ),
      ...
      m_acceptJobs( true )
{
    m_networkAccessManager.setProxy(RequestClient::getProxy());
...
{
    qDebug() << "HttpDownloadManager::addJob" << sourceUrl << destFileName << id << usage;
    if ( !d->m_acceptJobs ) {
...
{
    QString result( QStringLiteral("Mozilla/5.0 (compatible; Marble/%1; %2; %3; %4)") );
