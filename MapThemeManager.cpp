...
#include "geodata/parser/GeoSceneParser.h"
...
namespace
{
    static const QString mapDirName = QStringLiteral("maps");
...
{
    qWarning() << "MapThemeManager::Private::loadMapThemeFile" << mapThemeStringID;

    const QString mapThemePath = mapDirName + '/' + mapThemeStringID;
    qWarning() << "MapThemeManager::Private::loadMapThemeFile mapThemePath" << mapThemePath;

    const QString dgmlPath = MarbleDirs::path( mapThemePath );
    qWarning() << "MapThemeManager::Private::loadMapThemeFile dgmlPath" << dgmlPath;
    ...
    qDebug() << "MapThemeManager::Private::loadMapThemeFile finished" << mapThemeStringID;
...
QStringList MapThemeManager::Private::findMapThemes( const QString& basePath )
{
    ...
    QStringList mapPaths = paths.entryList( QStringList( QStringLiteral("*") ),
                                            QDir::AllDirs
                                            | QDir::NoSymLinks
                                            | QDir::NoDotAndDotDot );
    QStringList mapDirs;
    ...
    while ( it.hasNext() ) {
        ...
        QStringList tmp = QDir( themeDir ).entryList( QStringList( QStringLiteral("*.dgml") ),
...
QList<QStandardItem *> MapThemeManager::Private::createMapThemeRow( QString const& mapThemeID )
{
    ...
    if ( themeIconPixmap.isNull() ) {
        relativePath = QLatin1String("svg/application-x-marble-gray.png");
        themeIconPixmap.load( MarbleDirs::path( relativePath ) );
    }
    else {
...
void MapThemeManager::Private::addMapThemePaths( const QString& mapPathName, QStringList& result )
{
    ...
    QStringList orbDirNames = mapPath.entryList( QStringList( QStringLiteral("*") ),
                                                 QDir::AllDirs
                                                 | QDir::NoSymLinks
                                                 | QDir::NoDotAndDotDot );
    ...
    while ( itOrb.hasNext() ) {
        ...
        QStringList themeDirNames = orbPath.entryList( QStringList( QStringLiteral("*") ),
                                                       QDir::AllDirs
                                                       | QDir::NoSymLinks
                                                       | QDir::NoDotAndDotDot );
	    ...
	    QStringList themeFileNames = themePath.entryList( QStringList( QStringLiteral("*.dgml") ),
                                                              QDir::Files
                                                              | QDir::NoSymLinks );
