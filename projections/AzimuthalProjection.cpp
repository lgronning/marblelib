                                              ...
                                              QVector<QPolygonF> &polygons ) const
{
    ...
    const bool hasDetail = lineString.detail(0) != 0;

    while(itCoords < lineString.size())
    {

        double lon1;
        double lat1;

        lineString.getLonLat(itPreviousCoords, lon1, lat1, GeoDataCoordinates::Radian);

        double lon2;
        double lat2;

        lineString.getLonLat(itCoords, lon2, lat2, GeoDataCoordinates::Radian);

        // Optimization for line strings with a big amount of nodes
        bool skipNode = (hasDetail ? lineString.detail(itCoords) > maximumDetail
                : itCoords != 0 && isLong && !processingLastNode &&
                !viewport->resolves( lon1, lat1, lon2, lat2) );


        if ( !skipNode )
        {
            q->screenCoordinates( GeoDataCoordinates(lon2, lat2), viewport, x, y, globeHidesPoint );

            // Initializing variables that store the values of the previous iteration
            if ( !processingLastNode && itCoords == 0 )
            {
            ...
            if ( isAtHorizon )
            {
                GeoDataCoordinates previousCoords = lineString.at(itPreviousCoords);
                GeoDataCoordinates coords = lineString.at(itCoords);

                // Handle the "horizon case"
                horizonCoords = findHorizon( previousCoords, coords, viewport, f );
        ...
        if ( itCoords == lineString.size()  && lineString.isClosed() )
        {
            itCoords = 0;
