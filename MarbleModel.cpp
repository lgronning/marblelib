...
{
    QString mapThemeId = QLatin1String("");
...
void MarbleModelPrivate::addHighlightStyle(GeoDataDocument* doc)
{
    if ( doc ) {
            ...
            || highlightPenColor.isValid() )
        {
            ...
            styleMap.insert( QStringLiteral("highlight"), QStringLiteral("#").append(highlightStyle->id()) );
...
MarbleModelPrivate::MarbleModelPrivate()
    : m_clock(),
...
void MarbleModelPrivate::assignNewStyle( const QString &filePath, const GeoDataStyle::Ptr &style )
{
    ...
    styleMap.insert( QStringLiteral("normal"), QStringLiteral("#").append(style->id()) );
    doc->addStyleMap( styleMap );
    doc->addStyle( style );
    ...
    for ( ; iter != end; ++iter ) {
        if ( (*iter)->nodeType() == GeoDataTypes::GeoDataPlacemarkType ) {
            ...
            if ( placemark ) {
                ...
                {
                    placemark->setStyleUrl( QStringLiteral("#").append( styleMap.id() ) );
                }
            }
        }
    }
...
void MarbleModel::clearPersistentTileCache()
{
    ...
    if ( d->m_mapTheme->map()->hasTextureLayers() || d->m_mapTheme->map()->hasVectorLayers() ) {
            ...
            && !installMap.isEmpty() )
        {
            ...
            TileCreator tileCreator (sourceDir,
                                     installMap,
