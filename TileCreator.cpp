...
class TileCreatorPrivate
{
 public:
    TileCreatorPrivate( TileCreatorSource *source,
                        QString  dem, QString  targetDir=QString() )
       : m_dem(std::move( dem )),
...
public:
    ...
    QImage tile(int n, int m, int maxTileLevel) override
    {
        ...
        if ( stdImageWidth != imageWidth ) {
            mDebug() <<
            QStringLiteral( "TileCreator::createTiles() The size of the final image will measure  %1 x %2 pixels").arg(stdImageWidth).arg(stdImageHeight);
        }
...
TileCreator::TileCreator(const QString& sourceDir, const QString& installMap,
                         const QString& dem, const QString& targetDir, QObject *parent)
    : QObject(parent),
      d( new TileCreatorPrivate( nullptr, dem, targetDir ) )

{
    qDebug() << "Prefix: " << sourceDir
    ...
    if ( QDir::isAbsolutePath( sourceDir ) ) {
        ...
        qDebug() << "Trying absolute path*:" << sourcePath;
    }
    else {
    ...
    if ( d->m_targetDir.isNull() )
        d->m_targetDir = MarbleDirs::localPath() + "/maps/" + sourcePath.section( '/', -3, -2 ) + '/';
}

TileCreator::TileCreator(TileCreatorSource* source, const QString& dem, const QString& targetDir , QObject *parent)
    : QObject(parent),
...
void TileCreator::start()
{
    qDebug() << "TileCreator::start";
    ...
    qDebug() << QStringLiteral( "TileCreator::createTiles() image dimensions %1 x %2").arg(imageWidth).arg(imageHeight);

    if ( imageWidth < 1 || imageHeight < 1 ) {
        qWarning("Invalid imagemap!");
        return;
    }
    ...
    if ( maxTileLevel < 0 ) {
        qWarning()
        << QStringLiteral( "TileCreator::createTiles(): Invalid Maximum Tile Level: %1" )
        .arg( maxTileLevel );
    }
    ...
    QString  dirName( d->m_targetDir
                      + QStringLiteral("%1").arg(maxTileLevel) );
    if ( !QDir( dirName ).exists() ) 
        ( QDir::root() ).mkpath( dirName );

    for ( int n = 0; n < nmax; ++n ) {
        QString dirName( d->m_targetDir
                         + QStringLiteral("%1/%2").arg(maxTileLevel).arg( n, tileDigits, 10, QChar('0') ) );
        if ( !QDir( dirName ).exists() ) 
            ( QDir::root() ).mkpath( dirName );
    }
    ...
    for ( int n = 0; n < nmax; ++n ) {
        ...
        for ( int m = 0; m < mmax; ++m ) {
            ...
            tileName = d->m_targetDir + ( QStringLiteral("%1/%2/%2_%3.%4")
                                       .arg( maxTileLevel )
            ...
            if ( QFile::exists( tileName ) && d->m_resume ) {
                ...
                qDebug() << tileName << "exists already";
            ...
            else
            {
                ...
                if ( tile.isNull() ) {
                    qWarning() << "Read-Error! Null QImage!";
                    return;
                }
                ...
                if ( !ok )
                    qWarning() << "Error while writing Tile: " << tileName;

                qDebug() << tileName << "size" << QFile( tileName ).size();
    ...
    while( tileLevel > 0 ) {
        ...
        for ( int n = 0; n < nmaxit; ++n ) {
            ...
            for ( int m = 0; m < mmaxit; ++m ) {
                ...
                QString newTileName = d->m_targetDir + ( QStringLiteral("%1/%2/%2_%3.%4")
                                           .arg( tileLevel )
                                           .arg( n, tileDigits, 10, QChar('0') )
                                           .arg( m, tileDigits, 10, QChar('0') ) )
                                           .arg( d->m_tileFormat );
                ...
                {
                    qDebug() << newTileName << "exists already";
                }
                else
                {
                    tileName = d->m_targetDir + ( QStringLiteral("%1/%2/%2_%3.%4")
                    ...
                    {
                        qDebug() << "Tile write failure. Missing write permissions?";
                        emit progress( 100 );
                        return;
                    }
