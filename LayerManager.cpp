...
void LayerManager::renderLayers( GeoPainter *painter, ViewportParams *viewport )
{
    ...
    if ( d->m_showBackground ) {
        renderPositions << QStringLiteral("STARS") << QStringLiteral("BEHIND_TARGET");
    }

    renderPositions << QStringLiteral("SURFACE") << QStringLiteral("HOVERS_ABOVE_SURFACE") << QStringLiteral("ATMOSPHERE")
                    << QStringLiteral("ORBIT") << QStringLiteral("ALWAYS_ON_TOP") << QStringLiteral("FLOAT_ITEM") << QStringLiteral("USER_TOOLS");
    ...
    foreach( const auto& renderPosition, renderPositions ) {
        ...
        SortHelper::sort( layers.begin(), layers.end(), [] ( const LayerInterface * const one, const LayerInterface * const two ) -> bool {
            Q_ASSERT( one && two );
