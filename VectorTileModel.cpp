...
TileRunner::TileRunner( TileLoader *loader, const GeoSceneVectorTileDataset *texture, TileId id ) :
    m_loader( loader ),
    m_texture( texture ),
    m_id(std::move( id ))
{
...
void VectorTileModel::updateTile( const TileId &id, GeoDataDocument *document )
{
    ...
    if ( m_tileLoadLevel != id.tileLevel() ) {
        delete document;
        return;
    }

    document->setName(QStringLiteral("%1/%2/%3").arg(id.tileLevel()).arg(id.x()).arg(id.y()));
