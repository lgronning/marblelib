...
GeoDataPlacemark *OsmWay::create(const OsmNodes &nodes, QSet<qint64> &usedNodes) const
{
    ...
    if (isArea()) {
        ...
        coordinates.reserve(m_references.size());
        ...
        for (int i=0, n=m_references.size() - (stripLastNode ? 1 : 0); i<n; ++i) {
            ...
            coordinates.append(nodeIter.value().coordinates());
        }

        if (!isBuilding())
        {
            geometry = new GeoDataLinearRing(GeoDataLinearRing(coordinates).optimized());
        }
    }
    else
    {
