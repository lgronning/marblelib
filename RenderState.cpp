...
public:
    ...
    Private( QString name=QString(), RenderStatus status=Complete );
...
RenderState::Private::Private( QString name, RenderStatus status ) :
...
QString RenderState::Private::toString( const RenderState &state, int level ) const
{
    ...
    QString status;
    switch ( state.status() ) {
    case Marble::Complete:         status = QLatin1String("Complete"); break;
    case Marble::WaitingForUpdate: status = QLatin1String("Waiting for update"); break;
    case Marble::WaitingForData:   status = QLatin1String("Waiting for data"); break;
    case Marble::Incomplete:       status = QLatin1String("Incomplete"); break;
    }
    QString const name = ( state.name().isEmpty() ? QStringLiteral("Anonymous renderer") : state.name() );
    QString result = QStringLiteral("%1%2%3: %4").arg( prefix ).arg( indent ).arg( name ).arg( status );
