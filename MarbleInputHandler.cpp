...
const int TOOLTIP_START_INTERVAL = 200;
...
void
MarbleInputHandler::setMouseButtonPopupEnabled(Qt::MouseButton mouseButton, bool enabled)
{
    if (enabled)
    {
...
void MarbleDefaultInputHandler::lmbTimeout()
{
    if (!selectionRubber()->isVisible())
    {
         ...
         */
        if ( isPointOnGlobe ) {
            emit mouseClickGeoPosition( GeoDataCoordinates(clickedLon, clickedLat, 0,
                                        GeoDataCoordinates::Degree) );
        }
    }
}
...
void
MarbleDefaultInputHandler::showInfoTimeout()
{
    showInfo(d->mousePosition.x(), d->mousePosition.y());
}
...
void MarbleDefaultInputHandler::notifyPosition(bool isMouseAboveMap, qreal mouseLon, qreal mouseLat)
{
    ...
    {
        emit mouseMoveGeoPosition(GeoDataCoordinates(mouseLon, mouseLat));
    }
}
...
void MarbleDefaultInputHandler::adjustCursorShape(const QPoint &mousePosition, const QPoint &mouseDirection)
{
    ...
    if ((MarbleInputHandler::d->m_marblePresenter->map()->whichFeatureAt(mousePosition) == nullptr)
         && (!dataAction ))
    {
        if (!d->m_leftPressed)
        {
    ...
    else
    {
        if (!d->m_leftPressed)
        {
            d->m_arrowCur [1][1] = QCursor(Qt::ArrowCursor);
        }
    }
...
void
MarbleDefaultInputHandler::startShowInfo(const QPoint &mousePosition)
{
    if(d->moveTimer)
    {
        delete d->moveTimer;
    ...
    d->mousePosition = mousePosition;

    connect(d->moveTimer, SIGNAL(timeout()), this, SLOT(showInfoTimeout()));
    d->moveTimer->setSingleShot(true);
    d->moveTimer->start(10);
}

void MarbleDefaultInputHandler::stopShowInfo()
{
    if(d->moveTimer)
    {
        delete d->moveTimer;
...
bool MarbleDefaultInputHandler::handleMouseEvent(QMouseEvent *event)
{
    ...
    if (event->type() == QEvent::MouseMove && !d->m_leftPressed)
    {
